...
<div class="trabajador-form">
	 ...
	 <?php $form = ActiveForm::begin([ 'options'=>['layout' => 'horizontal', 'enctype'=>'multipart/form-data', 'id'=>'form1', 'action' => ['trabajador/load']]]); ?>
									...
									<td><?=$form->field($worker, "[$i]CALLE")->textInput()->label(false)   ?></td>
										...
										<td><?=$form->field($worker, "[$i]NUMERO_EXTERIOR")->textInput()->label(false)   ?></td>
											<td><?=$form->field($worker, "[$i]COLONIA")->textInput()->label(false)   ?></td>
												<td><?=$form->field($worker, "[$i]CODIGO_POSTAL")->textInput()->label(false)   ?></td><td><?php  $trabajador = new Trabajador();?>
